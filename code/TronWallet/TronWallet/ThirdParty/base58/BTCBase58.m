...
NSMutableData* BTCDataFromBase58CString(const char* cstring) {
    ...
    NSMutableData* bndata = nil;
    {
        ...
        if (bnsize <= 4) {
            ...
            BN_bn2mpi(&bn, (uint8_t *)bndata.mutableBytes);
...
NSMutableData* BTCDataFromBase58CheckCString(const char* cstring) {
    ...
    if (memcmp(hash.bytes, (const void *)((unsigned char*)result.bytes + length - 4), 4) != 0) {
        return nil;
    }
...
char* BTCBase58CStringWithData(NSData* data) {
    ...
    // Convert little endian data to bignum
    {
        ...
        unsigned char* bytes = (unsigned char*)mdata.mutableBytes;
    ...
    const unsigned char* pend = (unsigned char*)data.bytes + data.length;
    ...
    char* r = (char *)malloc(stringData.length);
